<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Inicio Noti LOL</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/css/materialize.min.css">
    <link rel="shortcut icon" href="/recursos/poro.png" type="image/x-icon">
    <link rel="stylesheet" href="style.css">
</head>
<body class="indigo lighten-5" center >


<nav class="nav-extended indigo darken-3">
  <div class="nav-wrapper">
    <h5 class="brand-logo">Noti-LOL</h5>
  </div>
  <div class="nav-content">
    <ul class="tabs tabs-transparent">
      <li class="tab"><a class="active" href="registro.php">Registro</a></li>
      <li class="tab"><a class="active" href="iniciar_sesion.php">Iniciar sesion</a></li>
    </ul>
  </div>
</nav>

  <div class="section">
    <h3 class = "col s12 center">Eventos del mes</h1>
    <div class="col s12 divider indigo darken-3 "></div>
  </div>
  <div class="row ">
    <div class="col s12 m6 offset-m3">
      <div class="video-container" ><iframe width="560" height="315" src="https://www.youtube.com/embed/eCVzb4PFI5Y?si=F6G6r_oSqHmaPArr" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div>
    </div>
  </div>
  
  <div class="section">
    <h3 class = "col s12 center">Reworks</h1>
    <div class="col s12 divider indigo darken-3 "></div>
  </div>
  <div class="row ">
    <div class="col s12 m6 offset-m3">
      <div class="video-container"><iframe width="560" height="315" src="https://www.youtube.com/embed/bh0t_Ba3HRw?si=HZOGWhSJis8p4fmY" title="YouTube video player" frameborder="" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div>
    </div>
  </div>

<footer class="page-footer indigo darken-3">
  <div class="container">
    <div class="row">
      <div class="col l6 s12">
        <h5 class="white-text">Nosotros</h5>
        <p class="grey-text text-lighten-4">Una página dedicada a subir el contenido más nuevo de League of Legends.</p>
      </div>
      <div class="col l4 offset-l2 s12">
        <h5 class="white-text">Redes sociales</h5>
        <ul>
          <li><a class="grey-text text-lighten-3" href="https://www.youtube.com/@leagueoflegends">YouTube</a></li>
          <li><a class="grey-text text-lighten-3" href="https://www.facebook.com/leagueoflegends">Facebook</a></li>
          <li><a class="grey-text text-lighten-3" href="https://www.leagueoflegends.com/es-mx/?fbclid=IwZXh0bgNhZW0CMTAAAR28SMainkr0ElTegyOAsXtHMmLAumF2wfF41b-yEr--C1as6SCgxI99NCg_aem_AQBgSHw-A0Trx5Y2YGUXundjsDNFDVLsQhFMvw6yzPPxjH3_BX-QXUPs4xNatmXkdZtEh0fsJJZdRIkr_HhI1S9q">Juega</a></li>
        </ul>
      </div>
    </div>
  </div>
  <div class="footer-copyright">
  </div>
</footer>
</body>
</html>


